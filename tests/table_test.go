...
func TestTable(t *testing.T) {
	...
	r = dryDB.Table("user as u").Columns("name").Find(&User{}).Statement
	...
	r = dryDB.Table("gorm.user").Columns("name").Find(&User{}).Statement
	if !regexp.MustCompile("SELECT .name. FROM .gorm.\\..user. WHERE .user.\\..deleted_at. IS NULL").MatchString(r.Statement.SQL.String()) {
		t.Errorf("Table with escape character, got %v", r.Statement.SQL.String())
	}

	r = dryDB.Columns("name").Find(&UserWithTable{}).Statement
	if !regexp.MustCompile("SELECT .name. FROM .gorm.\\..user. WHERE .user.\\..deleted_at. IS NULL").MatchString(r.Statement.SQL.String()) {
		t.Errorf("Table with escape character, got %v", r.Statement.SQL.String())
	}

	r = dryDB.Insert(&UserWithTable{}).Statement
	...
	r = dryDB.Table("(?) as u", DB.Model(&User{}).Columns("name")).Find(&User{}).Statement
	...
	r = dryDB.Table("(?) as u, (?) as p", DB.Model(&User{}).Columns("name"), DB.Model(&Pet{}).Columns("name")).Find(&User{}).Statement
	...
	r = dryDB.Where("name = ?", 1).Table("(?) as u, (?) as p", DB.Model(&User{}).Columns("name").Where("name = ?", 2), DB.Model(&Pet{}).Where("name = ?", 4).Columns("name")).Where("name = ?", 3).Find(&User{}).Statement
